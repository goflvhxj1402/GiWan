...
void MapTool::Save(HWND hDlg)
{
	...
	HANDLE hFile;
	DWORD Write;
	strcpy(lpstrFile, SelectMap(hDlg));
	if (lpstrFile == "")
		return;
	hFile = CreateFile(lpstrFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	for (int y = 0; y < MAP; y++)
		for (int x = 0; x < MAP; x++)
...
LPSTR MapTool::SelectMap(HWND hDlg)
{
	OPENFILENAME OFN;
	char lpstrFile[MAX_PATH] = "";
