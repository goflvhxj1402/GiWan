...
void Enemy::CreateTank()
{
	if (clock() - m_iCreateClcok >= CREATE_TIME && m_iCreateIndex < 3 && m_iLife > 0)
	{
		...
		m_iCreateIndex++;
		m_iLife--;
		if (rand() % 10 <= m_iLevel)//난이도에따라 강한탱크 생성확률조절
			TankPower = STRONG_ENEMY;
		tmpTank.ResetTank(TankPower, m_arrRespawnPt[RNum]);
...
void Enemy::UpdateUnit(HDC hdc, Map& Map)
{
	...
	for (int i = 0; i < m_vTank.size(); i++)
	{
		...
		{
			//정지아이템확인
			if (g_arrTankRect[i + 1].left == TANK_ITEM_PAUSE)
			{
				m_iPause = clock() + (PAUSE_TIME * 2);
				return;
			}
			//움직이기
		...
		else //정지상태이면 탱크Rect만 업데이트
			g_arrTankRect[i + 1] = m_vTank[i].GetTankRect();
		//플레이어기지파괴확인
...
void Enemy::Die()
{
	if (g_Index != EMPTY)
	{
		for (int i = 0; i < m_vTank.size(); i++)
		{
			...
			{
				if (m_vTank[i].GetTankType() == STRONG_ENEMY)
					m_vTank[i].ResetTank(ENEMY, m_vTank[i].GetTankPoint());
				else
				{
